...
def formatting_data():
    ...
    df = pd.read_csv('_Data/VALUE/precip.txt')
    ...
    df4 = pd.read_csv('_Data/VALUE/stations.txt')
    ...
    df7.to_csv('_Data/VALUE/value_rsamp.csv')
...
def all_gauge_data(minyear, maxyear, threshold=None):
    ...
    filepath = '_Data/VALUE/value_rsamp.csv'
    df = pd.read_csv(filepath)
    df = df.drop(['Unnamed: 0', 'alt'], axis=1)
