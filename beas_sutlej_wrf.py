...
def reformat_bannister_data():
    ...
    wrf_ds = xr.open_dataset(pwd + 'data/Bannister/Bannister_WRF.nc')
    ...
    wrf_ds.to_netcdf(pwd + 'data/Bannister/Bannister_WRF_raw.nc')

    # Bias corrected WRF data
    bc_ds = ds2.drop('m_precip')
    bc_ds = bc_ds.rename({'bias_corr_precip': 'tp'})
    bc_ds = interp(bc_ds)
    bc_ds.to_netcdf(pwd + 'data/Bannister/Bannister_WRF_corrected.nc')
