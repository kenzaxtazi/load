...
def eof_downloader(basin, all_var=False):
    ...
    eof1_z200_u = eof_formatter(
        pwd + "data/regional_z200_EOF1.nc", basin, name="EOF200U1"
    )
    eof1_z500_u = eof_formatter(
        pwd + "data/regional_z500_EOF1.nc", basin, name="EOF500U1"
    )
    eof1_z850_u = eof_formatter(
        pwd + "data/regional_z850_EOF1.nc", basin, name="EOF850U1"
    )

    eof2_z200_u = eof_formatter(
        pwd + "data/regional_z200_EOF2.nc", basin, name="EOF200U2"
    )
    eof2_z500_u = eof_formatter(
        pwd + "data/regional_z500_EOF2.nc", basin, name="EOF500U2"
    )
    eof2_z850_u = eof_formatter(
        pwd + "data/regional_z850_EOF2.nc", basin, name="EOF850U2"
    )
